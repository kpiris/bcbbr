...
if [ "${LASTEXPORT_VAULT_PER}" != "" ] ; then
    if [ ! -e "${LASTEXPORT_VAULT_PER}.sign" ] ; then
        echo -n "WARNING: \`${LASTEXPORT_VAULT_PER}' is not signed. Press <INTRO> to continue or Ctrl+c to abort :" >&2 ; read dummy
    else
...
if [ "${LASTEXPORT_VAULT_ORG}" != "" ] ; then
    if [ ! -e "${LASTEXPORT_VAULT_ORG}.sign" ] ; then
        echo -n "WARNING: \`${LASTEXPORT_VAULT_ORG}' is not signed. Press <INTRO> to continue or Ctrl+c to abort :" >&2 ; read dummy
    else
